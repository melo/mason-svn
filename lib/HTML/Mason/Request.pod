...
Note: under the current implementation, any pending filters (via the
<%filter> tag) will not be applied to the output when you abort.
This is a known bug but there is no easy workaround.
