...
=item * MasonDeclineDirs - boolean

Corresponds to the C<decline_dirs> parameter.
...
Alternatively you can use the L<HTML::Mason::Parser/allow_globals> parameter or method:
...
to run in (via the L<HTML::Mason::Parser/in_package> Parser parameter.)
...
Alternatively you can use the L<HTML::Mason::Interp/set_global> Interp method:
...
1. Set the L<HTML::Mason::ApacheHandler/decline_dirs> ApacheHandler parameter to 0.
...
L<HTML::Mason::Interp/code_cache_max_size> Interp parameter; default is 10MB.
...
To turn off code caching completely, set L<HTML::Mason::Interp/code_cache_max_size> to
0.
...
the L<HTML::Mason::Interp/use_object_files> Interp parameter to 0.

=head2 Preloading

You can tell Mason to preload a set of components in the parent
process, rather than loading them on demand, using the
L<HTML::Mason::Interp/preloads> Interp parameter.  Each child server will start
...
object files, using the L<HTML::Mason::Parser/make_component> Parser method.
...
You can activate this mode with the L<HTML::Mason::Interp/use_reload_file>
Interp method.
...
headers (see L<HTML::Mason::Devel/sending_http_headers>) and in handling
mid-request error conditions (see L<HTML::Mason::Request/clear_buffer>).
