...
Previous versions of Mason required you to use the C<handler.pl> file
to pass web requests to the Mason code.  This is no longer necessary,
as Mason can now be configured via your httpd.conf file.  However,
more complicated setups may still benefit from using custom code.

=head1 CONFIGURING MASON VIA httpd.conf

The configuration directives that Mason uses are all set via the
C<PerlSetVar> and C<PerlAddVar> directives provided by mod_perl.
Though these arguments are all strings, Mason treats them in different
ways.  The different argument types are:
...
that expect code references.  For example, an anonymous sub might look
like:

 PerlSetVar  MasonDieHandler  "sub { die join ' -- ', @_ }"

To call a named sub you have to be a little tricky and wrap it in an
anonymous subroutine, like this:

 PerlSetVar  MasonDieHandler  "sub { Carp::croak(@_) }"

Attempting to just pass a reference like this:

 PerlSetVar  MasonDieHandler  "\&Carp::croak"

B<does not work> correctly.  The problem is that it loses any
arguments that should be passed to the subroutine.  So don't try to do
this.

=item * list

Some parameters are multi-values, such as MasonPreloads.  To set those
simple use C<PerlSetVar> for the first value and then C<PerlAddVar>
for all subsequent values, like this:
...
The parameters that can be set are all the initialization parameters
for the B<L<HTML::Mason::Parser>>,
B<L<HTML::Mason::Interp>>, and
B<L<HTML::Mason::ApacheHandler>> objects.
These parameters are simply passed to the constructors for each of
these objects.  In addition, there are two more parameters that can be
set that affect Mason's behavior.  All parameters start with a
C<Mason> prefix.  The rest of the name is derived from the name of the
parameter given to the constructor.

All of the parameters are optional except for C<MasonCompRoot> and
C<MasonDataDir>.

The default values are the same as those when these parameters are
specified as object constructors and as such the defaults will not be
specified here.
...
If this parameter is true then Mason assumes that you may have
multiple sets of configuration directives defined, perhaps for
multiple locations or virtual hosts.  This means that Mason will
create a new set of parser/interp/handler objects for every request.
Otherwise Mason simply creates a single set at server startup to
maximize shared memory.

Defaults to false.

B<Important>: If you set this option, you must make sure that whatever
directories you are using as data directories (set via
C<MasonDataDir>) are writeable by the user that Apache runs as.
Similarly, your component roots must be readable by the user Apache
runs as.

This is only relevant when the C<MasonMultipleConfig> option is set as
true.
...
=item * MasonCompRoot - list (special)

This parameter is B<required>.  It correponds to the C<comp_root>
parameter.  This parameter can either be a single value, such as
...
See the L<HTML::Mason::Interp/comp_root> for more details on
...
=item * MasonDieHandler - code (special)

Corresponds to the C<die_handler> parameter.  B<NOTE>: This parameter
can also be set as C<0>.  See the L<HTML::Mason::Interp/die_handler> for more details.
