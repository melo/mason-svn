...
=head1 SYNOPSIS
    ...
    #
    PerlSetVar MasonCompRoot /opt/www/htdocs
    PerlSetVar MasonDataDir /opt/mason/data
...
These directives first tell Apache the location of your component root
and data directory.  The "Alias" then maps any "/mason" URLs into the
...
routine C<HTML::Mason::ApacheHandler::handler>.
...
The simplest remedy is to use a <FilesMatch> directive instead of a
<Location> or <Directory> directive, like this:
...
With this directive, only files ending in .html are processed by
Mason.  This can be nested inside a <Location> or <Directory>
directive if you wish.

=item Are you running with taint checks (e.g. PerlTaintCheck)?

If so, set the configuration variable C<MasonTaintCheck> as true.

=back

Both the component root and the data directory are set via the
httpd.conf file.
...
prefix. If not, recheck your Apache config files and also tail your
server's error log.
